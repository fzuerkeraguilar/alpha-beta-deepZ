...
def get_num_inputs_outputs(onnx_filename):
    ...
    inp_shape = tuple(d.dim_value if d.dim_value !=
                      0 else 1 for d in inp.type.tensor_type.shape.dim)
    out_shape = tuple(d.dim_value if d.dim_value !=
                      0 else 1 for d in out.type.tensor_type.shape.dim)
    ...
    return num_inputs, inp_shape, num_outputs, out_shape, inp_dtype
...
def read_vnnlib_simple(vnnlib_filename, num_inputs, num_outputs):
    ...
    for line in lines:
        ...
        if groups:
            ...
            for rv_tuple in rv:
                update_rv_tuple(rv_tuple, op, first, second,
        ...
        for rv_tuple in old_rv:
            for c in conjuncts:
                ...
                for i in range(count):
                    ...
                    update_rv_tuple(rv_tuple_copy, op, first,
                                    second, num_inputs, num_outputs)
    ...
    for rv_tuple in merged_rv.values():
        ...
        for d in range(num_inputs):
            ...
            assert r[0] != - \
                np.inf and r[1] != np.inf, f"input X_{d} was unbounded: {r}"
            box.append(r)
